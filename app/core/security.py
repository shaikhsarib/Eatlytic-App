...
Cryptographic signatures, cookie signing, rate limiting, and authorization verification.
"""
import os
import hmac
import hashlib
...
def get_device_key(request: Request, response: Response = None) -> str:
    ...
    """
    header_val = request.headers.get("X-Device-Key")
    if header_val:
        return header_val
    ...
    if response:
        ...
        is_secure = os.environ.get("ENVIRONMENT", "dev").lower() not in ("dev", "development", "local")
        response.set_cookie(
            "eatlytic_did",
            f"{new_did}:{sig}",
            httponly=True,
            secure=is_secure,
            samesite="lax",   # 'strict' breaks cross-origin redirects; lax is safer default
